...
export function TestimonialsSection() {
  ...
  // When we reach the end (index === total), instantly reset to 0 for seamless loop
  useEffect(() => {
    if (index === total) {
      setResetting(true);
      setIndex(0);
  ...
  // Auto-play: advance every 4 seconds in a loop
  useEffect(() => {
    ...
    <section className="bg-white py-16 lg:py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
          ...
          {/* Right - carousel with sliding track */}
          <div className="flex-1 flex items-stretch gap-4 w-full min-w-0">
            <button
              type="button"
              onClick={goPrev}
            ...
            <div className="flex-1 min-w-0 overflow-hidden">
              ...
              >
                {loopReviews.map((review, i) => (
                  <div
                    key={`${i}-${review.name}`}
                    className="flex-shrink-0 w-[280px] sm:w-[300px] md:w-[320px] rounded-lg p-4 min-h-[200px] flex flex-col bg-white border border-slate-100 shadow-sm"
                  >
